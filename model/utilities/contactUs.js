// no login
function contactUs(router,connection,sendgrid,config){
  var self=this;
  self.handleRoutes(router,connection,sendgrid,config);
}

contactUs.prototype.handleRoutes = function(router,connection,sendgrid,config){
        ...
        if(name == null || name == undefined || name == ''){
          ...
          if(message == null || message == undefined || message == ''){
            ...
            sendgrid.send({
              to:       config.cs_email,
              from:     email,
              subject:  'Contact Us - Customer <Subject : '+subject+'> <From : '+name+'>',
              text:     message,
              html:     "<p>"+message+"</p>",
            }, function(err, json) {
              if (err) {
              ...
              res.json({"message":"success sending contact-us to our customer services","error":"success","status":"contactus","jsonsgrid":json}); //status 0 berarti signup doang
              console.log(json);
            });
          }
        }
