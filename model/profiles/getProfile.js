...
getProfile.prototype.handleRoutes = function(router,connection){
  router.get("/getProfile/:id",function(req,res){
    ...
    if(idHost==null || idHost==undefined){
      ...
      var query = "select email,bussiness_category.category_name,host.category,company_desc,title,tipe.id_tipe,tipe.name as tipe,host.id_tipe from `host` join `tipe` on host.id_tipe=tipe.id_tipe join `bussiness_category` on host.category=bussiness_category.id_cat where host.id_host='"+idHost+"'";
