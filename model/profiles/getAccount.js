...
getAccount.prototype.handleRoutes = function(router,connection){
  router.get('/getAccount/:id',function(req,res){
    ...
    if(idHost == null || idHost == undefined || idHost == ''){
      ...
      connection.query("select email,company_name,about,handphone,city.city_name,host.location,address,img_base64 from `host` join `city` on host.location=city.id_city where id_host = "+idHost+"",function(err,rows){
        if(err){
